...
class SeqpareDB:
    ...
    def fetch_mask(self, label: str, distance_threshold: float) -> NDArray[np.bool]:
        ...
        bits: NDArray[np.bool] = np.zeros(len(rme.bed_names), dtype=np.bool)

        for label, value in data:
            assert label in rme.bed_names, f"unknown label: {label}"
            item_id = rme.bed_id(label)
    ...
    def labels_to_mask(self, positives: Iterable[str]) -> NDArray[np.bool]:
        ...
        @returns the mask
        """
        mask: NDArray[np.bool] = np.zeros(len(rme.bed_names), dtype=np.bool)
