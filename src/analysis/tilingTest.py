...
def trial(pipeline: EmbedPipeline, bed: IntervalDataset, tileSizes: Sequence[int]):
...
def tilingTest(pipeline: EmbedPipeline, beds: Sequence[IntervalDataset]):
    ...
    tileSizes = [size for size in range(meanSize // 3, round(meanSize * 1.4), meanSize // 10)]

    diffsList = list()

    for i, bed in enumerate(beds):
        diffsList.append(trial(pipeline, bed, tileSizes))
    ...
    ax.text(meanSize + 1.2, max(points) * 0.5, "mean interval size", fontsize=12, color="gray")
