...
def ctt(embeds, test_point_rate=.1, consideration_limit=10000):
    """
    ...
    if len(embeds) > consideration_limit:
        indices = np.random.choice(
            len(embeds), consideration_limit, replace=False)
        embeds = embeds[indices]

    # Further subsample for test points
    test_point_amnt = int(len(embeds) * test_point_rate)
    test_point_indices = np.random.choice(
        len(embeds), test_point_amnt, replace=False)
    test_points = embeds[test_point_indices]

    # Calculate distances for test embeddings
    # TODO: consider optimization with KNN (kd/ball?) trees
    dist_tests = euclidean_distances(test_points, embeds)  # matrix: all combos
    # Account for self-dist
    dist_tests[np.arange(len(test_points)), test_point_indices] = np.inf
    d_t = np.sum(np.min(dist_tests, axis=1))  # Get NN & sum dists at once

    # Generate random points
    embed_min = np.min(embeds, axis=0)
    embed_max = np.max(embeds, axis=0)
    embed_dim = embeds.shape[1]
    # TODO: adjust min max based on percetile to account
    # for distribution shape
    random_points = np.random.uniform(
        embed_min, embed_max, (len(test_points), embed_dim))

    # Calculate distances for random points
    dists_random = euclidean_distances(random_points, embeds)
    d_r = np.sum(np.min(dists_random, axis=1))
